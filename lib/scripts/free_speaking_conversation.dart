...
import 'package:flutter_doctor/scripts/subscriptiondialog.dart';
...
class FreeConversationchat extends StatefulWidget {
  ...
  FreeConversationchat({Key? key, required this.rol, required this.language,required this.subject}) : super(key: key?? UniqueKey());


  @override
  State<FreeConversationchat> createState() => _FreeConversationState();
}

class _FreeConversationState extends State<FreeConversationchat> {
  ...
  String _corrected_sentence="";
  var messages;
  var chat_messages=[{"role": "assistant", "content":"Hi, what do you want to talk about today?"}];//Change!!

  TextEditingController userInputTextEditingController = TextEditingController();

  Color _well_said= Colors.black;
  //SPEECH TO TEXT
  String _wordsSpoken="";
                  ...
                  if (userInputTextEditingController.text.length>0){
                    if(_sendColor== Colors.lightBlue){
                      ...
                      if(chat_messages.length<4){
                        if(chat_messages.length == 3){
                          globals.used_freeconversation=true;
                        }
                        send_message(userInputTextEditingController.text);
                      }else{
                        showDialog(context: context,builder: (BuildContext context) {return SubscriptionDialog(language:widget.language);},);
                      }
                    }
                  }else{
                    LengthError(context);
              ...
              builder: (BuildContext context, BoxConstraints viewportConstraints)
              {
                return Stack(
                    children: [
                      ...
                      Positioned(
                        right: viewportConstraints.maxHeight/40,
                      ...
                      if (_showText)
                        Positioned(
                          right: viewportConstraints.maxHeight/40+viewportConstraints.maxHeight / 20,
                          top: (viewportConstraints.maxHeight / 50),
                          child: AnimatedOpacity(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                'Voice Speed',
                                style: TextStyle(fontWeight: FontWeight.w500, color: Colors.purple[900]),
                              ),
                            ),
                          ),
                        ),
